...
model.load_state_dict(torch.load("saved_models/snela_skjut_9800.pth"))
target_model = DQN(3,32,action_space,game_variables)
# target_model = ViT((120, 160), 3, action_space, game_variables)
target_model.load_state_dict(torch.load("saved_models/snela_skjut_9800.pth"))
...
for episode in range(n_episodes):
    ...
    while not done:
        ...
        var = obs["gamevariables"]
        var2 = doom_game.get_game_variable(vizdoom.GameVariable.FRAGCOUNT)
    ...
    if loss and agent.loss < np.min(loss):
        # torch.save(agent.model.state_dict(), "saved_models/testing_best.pth")
        pass
    ...
    if episode % update_frequency == 0 and episode > 0: 
        ...
        agent.update_target_model()
        # torch.save(agent.model.state_dict(), f"saved_models/testing_{episode}.pth")
